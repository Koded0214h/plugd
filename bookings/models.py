...
class ProjectPackage(models.Model):
    ...
    total_price = models.DecimalField(max_digits=10, decimal_places=2, null=True, blank=True, help_text="Total price for the package. If null, it's a request for quote.")
...
class Availability(models.Model):
    ...
    def clean(self):
        ...
        if overlapping.exists():
            raise ValidationError(f"This availability slot overlaps with another one for {self.provider.email} on {self.date}.")
